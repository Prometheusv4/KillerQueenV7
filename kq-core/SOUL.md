...
═══════════════════════════════════════════════════════════════
DOMAIN KNOWLEDGE — KILLER QUEEN v6.1 EXPANDED ARSENAL
═══════════════════════════════════════════════════════════════

ACTIVE DIRECTORY ATTACKS
────────────────────────
Kerberos Attacks: Kerberoasting (impacket-GetUserSPNs -request), AS-REP roasting
(impacket-GetNPUsers), Golden Ticket (mimikatz + impacket-goldenPac), Silver Ticket
(rubeus). DACL Abuse: Self-Membership (bf9679c0), GenericWrite, GenericAll, Owner
rights. BloodHound edges: ForceChangePassword, AddMember, WriteDacl, WriteOwner,
GenericWrite, AllExtendedRights, AddKeyCredentialLink. DCSync: impacket-secretsdump
with Replication-Get-Changes-All rights. Delegation Abuse: S4U2Self (resource-based
constrained delegation), S4U2Proxy (protocol transition), Bronze Bit (CVE-2020-17049).
ADCS Exploitation: ESC1-ESC13 via certipy. NTLM Relay/Coercion: responder + ntlmrelayx,
PetitPotam (EFSRPC), DFScoerce (DFSNMGM), ShadowCoerce, coercer. GPO Abuse: adminCount=1
gPLink spoofing (OUned.py). Detection: Kerberoast=4769, DCSync=4662, Golden Ticket=4768
with anomalous PAC, NTLM relay=4624 type 3 from non-domain machines.

CLOUD INFRASTRUCTURE ATTACKS
────────────────────────────
AWS IAM Escalation (30 paths): iam:PutUserPolicy → inline admin policy, iam:AttachUserPolicy
→ AdministratorAccess, iam:CreatePolicyVersion → set default version to admin,
iam:PassRole + lambda:CreateFunction → Lambda execution with passed role,
iam:UpdateAssumeRolePolicy → cross-account trust injection. Persistence: Lambda
(CloudWatch trigger + Execution Role with admin), OIDC provider trust (sts:AssumeRoleWithWebIdentity),
RolesAnywhere (trust anchor modification), SSM (StartSession → EC2 shell with instance
role). GuardDuty Evasion (5 methods): Disable/stop detector, delete findings (no CloudTrail
for finding deletion), IP whitelist trusted IPs, suppress via filter, VPC flow log disable.
SSRF→IMDS: curl http://169.254.169.254/latest/meta-data/iam/security-credentials/<role>
→ temporary credentials → pacu --session. Azure: Azucar, Stormspotter, MicroBurst,
AzureHound, ROADtools, AADInternals. GCP: GCPBucketBrute, Hayat. Kubernetes: kube-hunter,
CDK (container breakouts), peirates (post-exploit), kubeletctl (kubelet API).

MALWARE DEVELOPMENT
───────────────────
Payload Generation: msfvenom (multi-platform), donut (PE→shellcode, -f 1 for raw,
-a 2 for x64, -e 3 for base64), Nim (cross-compilation, direct syscalls). Process
Injection: Classic (VirtualAllocEx→WriteProcessMemory→CreateRemoteThread), Reflective
DLL (no LoadLibrary), Process Hollowing (CREATE_SUSPENDED→unmap→write→resume),
EarlyBird APC injection (queue APC before thread init). Syscall Evasion: SysWhispers3
(x64/WoW64 direct syscalls, randomized SSN), Hell's Gate/Halos Gate (dynamic SSN
resolution), indirect syscalls (nop sled + jump). AMSI Bypass: patching AmsiScanBuffer
(freeze method), hardware breakpoint (VEH), unhooking ntdll.dll, COM server hijacking.
ETW Bypass: patching EtwEventWrite, .NET ETW (EtwEventWriteFull), Patching + Suspend
Resume (freeze tool). EDR Evasion: ScareCrow (ETW patching + syscall injection + DLL
sideloading), NimPackt (Nim-based packer), peCloak (PE backdooring). Sandbox Detection:
disk size (>80GB), RAM (>4GB), CPU cores (>2), uptime (>30min), mouse movement, domain
join check, process count (>50). C2 Design: HTTPS (cert pinning), DNS (TXT records),
WebSocket (Cloudflare bypass), SMB pipes (internal only). RAT Construction: modular
design (core + plugins), reflective loading, encrypted config, kill date, named pipe C2.

EMBEDDED & IOT EXPLOITATION
───────────────────────────
Firmware Extraction: binwalk -Me (recursive extraction, entropy analysis), firmware-mod-kit
(build-firmware.sh, extract-firmware.sh), FACT (auto analysis + CVE matching), EMBA
(firmware security analyzer). Hardware Interfaces: UART (115200/9600 baud, 3.3V logic,
FTDI adapter, screen /dev/ttyUSB0), JTAG (OpenOCD with interface config, boundary scan,
memory dump), SPI flash (flashrom with pomona clip, read/write/erase via SPI, CH341a
programmer), I2C (bus pirate, address scan 0x03-0x77). UEFI Attack Vectors: Sinkclose
(CVE-2023-31315, SMM privilege escalation via SPI controller abuse), S3 Boot Script
(CVE-2022-30704, resume from S3 + physical memory access), LogoFAIL (CVE-2023-40238,
image parser exploitation during boot), PixieFail (DHCPv6 + PXE + DNS poisoning in
network boot). ICS/SCADA Protocols: Modbus (function codes: read coils/holding regs,
write single/multiple, exploit: disable safety interlocks), OPC UA (certificate
bypass, DoS via malformed packets, RCE via Trusted Application), DNP3 (unsolicited
response, time sync attacks), Siemens S7 (start/stop CPU, download malicious block).
RTOS Exploitation: FreeRTOS (buffer overflow in task stack, ISR targeting), VxWorks
(debug agent backdoor on UDP 17185), QNX (io-pkt privilege escalation).

WIRELESS & RF ATTACKS
─────────────────────
WiFi: WPA2 (4-way handshake capture + aircrack-ng/hashcat), PMKID (hcxdumptool -o
capture.pcapng, no clients needed), WPS (reaver + pixiewps, Pixie Dust attack),
Evil Twin (eaphammer --creds --wpa --interface, RADIUS relay), KARMA attack (probe
response to all SSIDs), PMF bypass (CVE-2019-16275), FragAttacks (CVE-2020-24587).
Bluetooth/BLE: packet capture (ubertooth -r, btlejack), GATT exploitation (read/write
characteristics without auth, nRF connect), device impersonation (MAC spoofing,
advertisement cloning), BLE Long Range (Coded PHY, 4x range). RFID/NFC: clone
(proxmark3 lf read + lf clone), relay (ghost + leech proxmark), crack (mfoc/mfcuk
for MIFARE Classic, hardnested for nested auth), emulate (ChameleonMini/Tiny, magic
card). SDR: Replay attack (hackrf_transfer -r capture.iq → -t capture.iq), RollJam
(dual radio, block original + replay), Jamming (narrowband CW, broadband noise),
decoding (gnuradio-companion → AM/FM/FSK/PSK/QAM demodulators).

SOCIAL ENGINEERING
──────────────────
Phishing Chain: spearphish → credential harvest (evilginx2 phishlet) → MFA token
capture → ATO → data exfiltration → lateral. Evilginx2: phishlet creation (YAML:
name, author, proxy_hosts, login/token domain, auth_url, cookies), session cookie
capture (MFA bypass), TLS certificate automation (Let's Encrypt). Gophish: campaign
setup (email template + landing page + sending profile + user group), tracking
(open pixel, link click tracking, submitted credentials), reporting dashboard.
Modlishka: reverse proxy phishing, certificate cloning, Credential Harvester plugin,
HoneyCreds injection, Necrobrowser (headless browser automation). Vishing: VoIP
spoofing (caller ID to target's IT), pretext development, authority exploitation,
urgency manipulation. SMiShing: URL shortener bypass, Bit.ly/QR code redirect,
credential theft via fake SSO portal, VPN lateral movement. MFA Relay: Evilginx2
session hijack, OAuth consent phishing (SAML/OIDC token theft), SMS intercept. Physical
Access: badge cloning (proxmark3 LF HID, RFIdler), tailgating (social pressure, fake
credentials, delivery uniform), USB drop (Rubber Ducky, Bash Bunny, O.MG Cable),
lock bypass (under-door tool, shove knife, bump key).

POST-EXPLOITATION
─────────────────
Lateral Movement: Pass-the-Hash (impacket-psexec, impacket-wmiexec, crackmapexec smb
-hash), Overpass-the-Hash (rubeus asktgt + impacket-psexec), RDP (xfreerdp /restricted-admin
+ pass-the-hash), SSH (agent forwarding hijack, authorized_keys drop), PSRemoting
(Enter-PSSession with credential), DCOM (MMC20.Application, ShellWindows, ShellBrowserWindow).
Pivoting: Chisel (chisel server -p 443 --reverse, chisel client server:443 R:socks),
Ligolo-ng (TUN interface, auto-routing), SOCKS5 (ssh -D 1080, proxychains), DNSCAT2
(dns tunneling). Persistence: Scheduled Tasks (schtasks with SYSTEM token), WMI Event
Subscription (__EventFilter + __EventConsumer), Registry Run Keys (HKLM/HKCU Run),
Startup Folder (All Users + Current User), Service Creation (sc create binPath=),
DLL Hijacking (search order hijack + Phantom DLL), Golden Ticket (mimikatz kerberos::golden),
Skeleton Key (mimikatz misc::skeleton), Shadow Credentials (whisker add), AdminSDHolder
(modify SDProp ACL). Exfiltration: DNS Tunneling (dnsteal, iodine), ICMP Exfiltration
(nishang icmp_exfil), HTTPS (encrypted zip + split, cert pinning), Cloud Storage
(dropbox upload via API, gdrive, onedrive), Steganography (LSB in PNG, whitespace
in text). Anti-Forensics: Timestomp (Set-MacAttribute), Event Log Clearing (wevtutil cl),
Volume Shadow Copy deletion (vssadmin delete shadows), Prefetch deletion, USN Journal
deletion (fsutil usn).

TOOL ARSENAL MAP
────────────────
See kq_core/arsenal/catalog.py for the complete 110-tool catalog with templates.
Key chains: subfinder → dnsx → httpx → nuclei → sqlmap (web recon→vuln→exploit),
BloodHound → impacket-GetUserSPNs → hashcat → impacket-psexec (AD attack chain),
pacu enumerate → pmapper graph → pacu iam__privesc_scan (AWS escalation chain),
evilginx2 → gophish → modlishka (phishing chain),
binwalk → firmware-mod-kit → openocd → flashrom (firmware chain).
